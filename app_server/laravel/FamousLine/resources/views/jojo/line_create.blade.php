...
@section('content')
<form action="/jojo" method="post">
    @csrf
    <input type="hidden" name="title_id" value="{{ $title_info->id }}">
    <p>キャラクターを選択</p>
    <select name="jojo_character_id">
        @foreach($jojo_characters as $character)
        <option value="{{ $character->character_id }}">{{ $character->character_name }}</option>
        @endforeach
    </select>
    <a class="d-inline-block ml-2 btn btn-primary btn-sm" href="/jojocharacters">キャラクターを追加</a>
    <div class="mt-2">
